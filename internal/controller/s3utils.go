...
type S3FailoverMarker struct {
	...
	Phase           string `json:"phase"` // "initiated", "recovering", "completed"
...
const (
	...
	FailoverPhaseInitiated  = "initiated"
	FailoverPhaseRecovering = "recovering"
	FailoverPhaseCompleted  = "completed"
	// TTL is only a safety fallback - marker should be explicitly removed
	// Set to 24 hours to handle cases where removal fails
...
type ObjectStorer interface {
	...
	RemoveFailoverMarkerForVRG(pathPrefix, expectedCluster string) error
	UpdateFailoverMarkerPhase(pathPrefix string, phase string) error
}
...
func (s *s3ObjectStore) RemoveFailoverMarkerForVRG(pathPrefix, expectedCluster string) error {
	markerKey := pathPrefix + failoverMarkerKey
	...
	if marker.FailoverCluster != expectedCluster {
		return fmt.Errorf("refusing to remove marker for different cluster: marker is for %s, current cluster is %s (generation: %d)",
			marker.FailoverCluster, expectedCluster, marker.MarkerGeneration)
	}

	err := s.DeleteObject(markerKey)
	if err != nil && !isAwsErrCodeNoSuchKey(err) {
		return fmt.Errorf("failed to remove failover marker: %w", err)
	}
...
func (s *s3ObjectStore) UpdateFailoverMarkerPhase(pathPrefix string, phase string) error {
	markerKey := pathPrefix + failoverMarkerKey
